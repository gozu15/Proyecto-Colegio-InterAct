   ...
   //captura la excepcion
    ...
    public int Update(Paralelo paralelo) {
        ...
        try {
            ...
            st = c.prepareStatement(
                    "UPDATE paralelo SET nombre_paralelo = ? WHERE idPARALELO = ?");
            // st.setObject(1, null); //el 1 indica que se sustituira el primer '?' con el
            // valor en int de 1234
            st.setString(1, paralelo.getNombre()); // el 2 indica que se sustituira el segundo '?' por el valor en double
                                                  // de 123.45
            st.setInt(2, paralelo.getId());
            // el 3 indica que se sustiruira el tercer '?' por la cadena
                                                       // "hola
    ...
    public int Delete(int id) {
        ...
        try {
            ...
            st = c.prepareStatement("DELETE FROM paralelo WHERE  idPARALELO = ?");
            // st.setObject(1, null); //el 1 indica que se sustituira el primer '?' con el
            // valor en int de 1234
